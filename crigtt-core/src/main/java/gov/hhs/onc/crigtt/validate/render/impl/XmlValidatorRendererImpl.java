...
public class XmlValidatorRendererImpl extends AbstractValidatorRenderer implements XmlValidatorRenderer {
    @Resource(
            name = "objFactoryValidate"
    )
    private ObjectFactory objFactory;
    @Resource(
            name = "jaxbMarshallerValidate"
    )
    ...
    protected byte[] renderInternal(ValidatorResponse resp, Map<String, Object> opts) throws Exception {
        return (CrigttOptionUtils.toBoolean(opts.get("format"), (Boolean)this.defaultOpts.get("format")) ? this.displaySerializer : this.serializer).serializeToBytes(new ByteArraySource(this.jaxbMarshaller.marshal(resp instanceof ValidatorReport ? this.objFactory.createReport((ValidatorReport)resp) : this.objFactory.createError((ValidatorError)resp))));
    }
}
